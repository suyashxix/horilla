/**
 * General Invoice Detail Management JavaScript
 */

class GeneralInvoiceDetailManager {
    constructor() {
    ...
    init() {
        ...
        console.log('General Invoice Detail Manager initialized');
    }

    getInvoiceIdFromUrl() {
        const pathParts = window.location.pathname.split('/');
        const invoicesIndex = pathParts.indexOf('general-invoices');
    ...
    async markGeneralAsPaid() {
        if (!confirm('Are you sure you want to mark this general invoice as paid?')) {
            return;
        }

        try {
            const response = await fetch(`/invoicing/general-invoices/${this.invoiceId}/mark-paid/`, {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                this.showToast('General invoice marked as paid successfully!', 'success');
                setTimeout(() => {
                    location.reload();
                }, 1000);
            } else {
            ...
            this.showToast('Error marking general invoice as paid', 'error');
        }
    }
    ...
    async sendGeneralEmail() {
        ...
        try {
            const response = await fetch(`/invoicing/general-invoices/${this.invoiceId}/send-email/`, {
                method: 'POST',
                headers: {
                    'X-CSRFToken': this.getCsrfToken(),
                },
            ...
            if (data.success) {
                this.showToast('General invoice sent successfully!', 'success');
            ...
            console.error('Error:', error);
            this.showToast('Error sending general invoice', 'error');
        } finally {
            // Reset button state
    ...
    async uploadGeneralPdf() {
        ...
        try {
            const response = await fetch(`/invoicing/general-invoices/${this.invoiceId}/upload-pdf/`, {
                method: 'POST',
                body: formData,
                headers: {
    ...
    async generateGeneralPdf() {
        try {
            ...
            const response = await fetch(`/invoicing/general-invoices/${this.invoiceId}/download-pdf/`, {
                method: 'GET',
            });
...
function markGeneralAsPaid(invoiceId) {
    if (window.generalInvoiceDetailManager) {
        window.generalInvoiceDetailManager.markGeneralAsPaid();
    }
}

function sendGeneralEmail(invoiceId) {
    if (window.generalInvoiceDetailManager) {
        window.generalInvoiceDetailManager.sendGeneralEmail();
    }
}

function uploadGeneralPdf(invoiceId) {
    if (window.generalInvoiceDetailManager) {
        window.generalInvoiceDetailManager.uploadGeneralPdf();
    }
}

function generateGeneralPdf(invoiceId) {
    if (window.generalInvoiceDetailManager) {
        window.generalInvoiceDetailManager.generateGeneralPdf();
    }
}

// Initialize when DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    window.generalInvoiceDetailManager = new GeneralInvoiceDetailManager();
});
